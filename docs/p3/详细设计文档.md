...
本文档整合了校园互助服务平台详细设计阶段的所有产出，包括类图设计、API 规范、数据库设计（ER 图 + 建表 SQL）以及实际工程中的设计模式应用。本文档内容与项目实际代码一致（基于 Flyway V1–V11 数据库迁移和 MyBatis-Plus 实体模型）。
...
> 完整类图（Mermaid 格式，含全部字段和关系）请见 `类图.md`。
...
|---|------|------|------|-------------|------|------|
| 1 | POST | `/register` | 无 | `RegisterRequest` (studentId, password, name) | `ApiResult<Void>` | 注册，自动创建账户+隐私配置+发放注册积分 |
| 2 | POST | `/login` | 无 | `LoginRequest` (studentId, password) | `ApiResult<LoginResponse>` (token, userId, name, role, avatar) | 登录，返回 JWT |
| 3 | GET | `/profile` | JWT | — | `ApiResult<UserInfoResponse>` | 获取当前用户完整信息（含积分、信誉、隐私设置） |
| 4 | PUT | `/profile` | JWT | `UpdateProfileRequest` (name, avatar, isAnonymous, maskName) | `ApiResult<UserInfoResponse>` | 更新个人资料与隐私设置 |
| 5 | POST | `/avatar` | JWT | `MultipartFile file` | `ApiResult<String>` (URL) | 上传头像，返回文件 URL |
| 6 | PUT | `/password` | JWT | `ChangePasswordRequest` (oldPassword, newPassword) | `ApiResult<Void>` | 修改密码，需验证旧密码 |

#### 3.4.2 需求模块 — `DemandController`（`/api/v1/demands`）

| # | 方法 | 路径 | 认证 | 请求体/参数 | 响应 | 说明 |
|---|------|------|------|-------------|------|------|
| 7 | POST | `/` | JWT | `CreateDemandRequest` (type, title, description, location, deadline, rewardType, rewardAmount, isAnonymous, images, attributes) | `ApiResult<DemandResponse>` | 发布需求；积分类型自动冻结报酬；组队类型自动加入发布者为队长 |
| 8 | POST | `/upload-image` | JWT | `MultipartFile file` | `ApiResult<String>` (URL) | 上传需求图片 |
| 9 | GET | `/` | JWT | `pageNum, pageSize, type?, keyword?, sortBy?` | `ApiResult<Page<DemandResponse>>` | 需求列表（支持按类型筛选、关键词搜索、按时间/报酬排序） |
| 10 | GET | `/my` | JWT | `role` (publisher/acceptor) | `ApiResult<List<DemandResponse>>` | 我发布/接取的需求列表 |
| 11 | GET | `/my/team` | JWT | — | `ApiResult<List<DemandResponse>>` | 我加入的组队需求列表 |
| 12 | GET | `/{demandId}` | JWT | — | `ApiResult<DemandResponse>` | 需求详情（含发布者信息、团队成员、匿名处理） |
| 13 | PUT | `/{demandId}/accept` | JWT | — | `ApiResult<DemandResponse>` | 接单（仅 OPEN 状态，组队类型需走申请流程） |
| 14 | PUT | `/{demandId}/complete` | JWT | — | `ApiResult<DemandResponse>` | 完成需求（仅发布者，积分从冻结转入接单者账户） |
| 15 | PUT | `/{demandId}/cancel` | JWT | — | `ApiResult<Void>` | 取消需求（仅发布者，退回冻结积分） |

#### 3.4.3 组队模块 — `TeamMemberController`（`/api/v1/demands/{demandId}/team`）
...
#### 3.4.4 聊天模块 — `ChatController`（`/api/v1/chat`）
...
#### 3.4.5 评价模块 — `EvaluationController`（`/api/v1/evaluations`）
...
#### 3.4.6 通知模块 — `NotificationController`（`/api/v1/notifications`）
...
#### 3.4.7 积分模块 — `PointsController`（`/api/v1/points`）
...
#### 3.4.8 管理模块 — `AdminController`（`/api/v1/admin`）

| # | 方法 | 路径 | 认证 | 说明 |
|---|------|------|------|------|
| 42 | GET | `/users` | JWT + ADMIN | 用户列表（支持关键词搜索，分页） |
| 43 | PUT | `/users/{userId}/status` | JWT + ADMIN | 封禁/解封用户（status: 0/1） |
...
数据库采用 MariaDB/MySQL（utf8mb4 字符集），共 11 张表，通过 Flyway 版本化迁移管理（V1–V11）。
...
| 序号 | 交付物 | 文件位置 | 状态 |
|------|--------|----------|------|
| 1 | 类图 | `类图.md` | ✅ v2.0 — 基于实际 MyBatis-Plus 实体 |
| 2 | SOLID 检查清单 | `SOLID检查清单.md` | ✅ 记录 AI 原始设计违规及修正 |
| 3 | API 规范文档 | `API 规范文档.yaml` | ✅ OpenAPI 3.0 格式 |
| 4 | ER 图 | `ER图.md` | ✅ v2.0 — 基于实际 11 表 schema |
| 5 | 建表 SQL | `建表SQL.sql` | ✅ 可执行建表语句 |
| 6 | AI 协作反思日志 #3 | `AI 协作反思日志 #3.md` | ✅ |
| 7 | 详细设计文档（整合版） | 本文档 | ✅ v2.0 |
